...
namespace Huzur_Vinç
{
    public partial class AnaSayfa : System.Web.UI.Page
    {
        ...
        {
            SqlCommand commandListSlides = new SqlCommand("Select * from TableSlide where SlaytID=@p1", SqlConnectionClass.con);

            SqlConnectionClass.CheckConnection();

            commandListSlides.Parameters.AddWithValue("@p1", 1);

            SqlDataReader dr = commandListSlides.ExecuteReader();

            while (dr.Read())
            {
                Image1.ImageUrl = dr[1].ToString();
                Image2.ImageUrl = dr[2].ToString();
                Image3.ImageUrl = dr[3].ToString();
                Image4.ImageUrl = dr[4].ToString();
                Image5.ImageUrl = dr[5].ToString();
                Image6.ImageUrl = dr[6].ToString();
                Image7.ImageUrl = dr[7].ToString();
            }
            ...
            SqlCommand commandGetNews = new SqlCommand("SELECT TOP 3 * FROM TableNews ORDER BY NewsID DESC", SqlConnectionClass.con);

            SqlConnectionClass.CheckConnection();

            SqlDataReader drNews = commandGetNews.ExecuteReader();

            DataList2.DataSource = drNews;
